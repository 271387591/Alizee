...
function deleteUser(id,modal){
    var ids=[];
    ids.push(id);
    var result=requestStringData('html/food/security/delete',{ids:ids.join(',')});
    if(result.success){
        alertSuccess('操作成功');
        listTable();
...
function edit(id){
    if(id){
        $('#mainContent').load(appPath+"html/food/security/addFood?id="+id);
    }else{
        $('#mainContent').load(appPath+"html/food/security/addFood")
    }
}
